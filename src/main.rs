    ...
    loop {
        ...
        match serde_json::from_str::<Move>(&received_str) {
            Ok(game_move) => {
                ...
                if let Err(err) = process_move(&mut game, game_move, role, &clients) {
                ...
                if let Some(winner) = game.winner {
                    ...
                    if games_played < 50 {
                        ...
                        *game = GameState::new(1); // Reset the game state
                        drop(game);
                        initialize_game(&game_state, &clients);
                    } else {
...
fn initialize_game(
    ...
    let mut file = File::create("board_state.txt").expect("Unable to create file");
    let board_state_json = serde_json::to_string(&response.board_state).expect("Unable to serialize board state");
